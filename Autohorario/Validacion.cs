    ...
    internal class Validacion
    {
        ...
        private static List<(string, int)> Validarhoras(string codigo_asignatura, int id_profesor, int id_seccion, List<(string, int)> h_s)
        {
            ...
            using (SqlCommand cmd = new SqlCommand("ppCheck_schedule", con))
            {
                ...
                for (int i = 0; i < horario_seleccionado.Count; i++)
                {
                    hora_inicio_asignatura = int.Parse(horario_seleccionado[i].Item1.Substring(0, 2));
                    hora_fin_asignatura = int.Parse(horario_seleccionado[i].Item1.Substring(3, 2));
                    if (hora_inicio_asignatura > 13 && hora_inicio_asignatura % 2 != 0 && hora_fin_asignatura - hora_inicio_asignatura > 1)
                    {
                        horario_seleccionado[i] = ($"{hora_inicio_asignatura + 1}/{hora_fin_asignatura}", horario_seleccionado[i].Item2);
                    }
                }
