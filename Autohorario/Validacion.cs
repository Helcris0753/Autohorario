...
namespace Autohorario
{
    internal class Validacion
    {
        ...
        public static void Getdata(string codigo_asignatura, int id_seccion, int creditos_asignatura, List<(string, int)> horario_seleccionado_presencial, int modalidad, int id_profesor, List<(string, int)> horario_semanal_disponible,List<(string, int)> horario_seleccionado_virtual)
        {
            //listas necesarias para ser rellenadas de informacion
            List<(string, int)> horario_presencial_disponible = new List<(string, int)>();
            List<(string, int)> horario_virtual_disponible = new List<(string, int)>();

            //Se pasa horario seleccionado junto con otras informaciones para rellenar horario disponible
            horario_presencial_disponible = Validarhoras(codigo_asignatura, id_profesor, id_seccion,horario_seleccionado_presencial);

            horario_virtual_disponible = Validarhoras(codigo_asignatura, id_profesor, id_seccion, horario_seleccionado_virtual);
            ...
            Insercion.data_insercion(horario_presencial_disponible, horario_semanal_disponible, horario_virtual_disponible, horario_seleccionado_presencial, id_seccion, creditos_asignatura, modalidad);
        }
        ...
        private static List<(string, int)> Validarhoras(string codigo_asignatura, int id_profesor, int id_seccion, List<(string, int)> h_s)
        {
            ...
            List<(string, int)> horario_presencial = h_s.ToList();
            ...
            using (SqlCommand cmd = new SqlCommand("ppCheck_schedule", con))
            {
                ...
                for (int i = 0; i < horario_presencial.Count; i++)
                {
                    ...
                    for (int j = 0; j < horas_ocupadas.Count; j++)
                    {
                        ...
                        if (horario_presencial[i].Item2 == horas_ocupadas[j].Item2)
                        {
                            ...
                            if (hora_inicio_asignatura <= hora_inicio_seleccion && hora_fin_asignatura < hora_fin_seleccion && hora_inicio_seleccion < hora_fin_asignatura)
                            {
                                ...
                                horario_presencial[i] = ($"{Insercion.zero(hora_fin_asignatura)}/{Insercion.zero(hora_fin_seleccion)}", dia);
                            }
                            //si las horas ocupadas se solapan de tal modo:
                            // __________       (horas seleccionadas)
                            //      _________   (horas ocupadas)
                            ...
                            {
                                horario_presencial[i] = ($"{Insercion.zero(hora_inicio_seleccion)}/{Insercion.zero(hora_inicio_asignatura)}", dia);
                            }
                            //si las horas ocupadas se solapan de tal modo:
                            ...
                            {
                                horario_presencial[i] = ($"{Insercion.zero(hora_inicio_seleccion)}/{Insercion.zero(hora_inicio_asignatura)}", dia);
                                horario_presencial.Add(($"{Insercion.zero(hora_fin_asignatura)}/{Insercion.zero(hora_fin_seleccion)}", dia));
                            }
                            ...
                            {
                                horario_presencial[i] = ("00/00", dia);
                            }
                        }
                    }
                }
                ...
                horario_disponible = horario_presencial.Where(horario => int.Parse(horario.Item1.Substring(3, 2)) - int.Parse(horario.Item1.Substring(0, 2)) > 0 ).ToList();
